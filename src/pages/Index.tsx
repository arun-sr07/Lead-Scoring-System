...
export default function Index() {
  ...
  const uploadLeads = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to upload leads');
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card className="p-6 space-y-4 bg-card/50 backdrop-blur border-primary/10 shadow-glow">
            ...
            <div className="space-y-2">
              ...
              <Input id="offer-name" value={offerName} onChange={(e) => setOfferName(e.target.value)} placeholder="AI Sales Assistant Pro" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="value-props">Value Propositions (comma-separated)</Label>
              <Textarea id="value-props" value={valueProps} onChange={(e) => setValueProps(e.target.value)} placeholder="Automate follow-ups, Increase conversions" rows={3} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="use-cases">Ideal Use Cases (comma-separated)</Label>
              <Textarea id="use-cases" value={useCases} onChange={(e) => setUseCases(e.target.value)} placeholder="B2B SaaS, Sales teams" rows={3} />
            </div>
          ...
          <Card className="p-6 space-y-4 bg-card/50 backdrop-blur border-primary/10 shadow-glow">
            ...
            <Button onClick={uploadLeads} disabled={!csvFile} className="w-full">Upload Leads</Button>
            <div className="pt-4 border-t border-border/50">
              <Button onClick={runScoring} disabled={loading} className="w-full bg-gradient-to-r from-primary to-accent hover:opacity-90">
                ...
                <tbody className="divide-y divide-border/50">
                  {results.map((result, idx) => (
                    <tr key={idx} className="hover:bg-accent/5">
                      ...
                      <td className="py-3">{result.role}</td>
                      <td className="py-3">{result.company}</td>
                      <td className="py-3">
                        <span className={`px-2 py-1 rounded text-xs ${result.intent === 'High' ? 'bg-green-500/20 text-green-300' : 'bg-yellow-500/20 text-yellow-300'}`}>
                          {result.intent}
                        </span>
                      </td>
                      <td className="py-3">{result.score}</td>
                      <td className="py-3 text-xs max-w-xs truncate">{result.reasoning}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
